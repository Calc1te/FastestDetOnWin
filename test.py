...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(opt.weight, map_location=device, weights_only=True))
    ...
    for box in output[0]:
        ...
        # 检查类别索引是否在有效范围内
        if category_index < 0 or category_index >= len(LABEL_NAMES):
            print("类别索引超出范围:", category_index)
            continue  # 跳过该框

        category = LABEL_NAMES[category_index]
    ...
    cv2.imwrite("result000.png", ori_img)
